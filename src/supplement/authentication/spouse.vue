<template>
	<div id="spouse" class="spouse" style="padding-bottom: 60px;">
    <van-cell-group>
      ...
      <van-cell class="vanCell"><van-field input-align="right" label="银行预留电话" v-model="bankPhone" placeholder="请输入银行预留电话" /></van-cell>
		...
		<van-row class="spouse-nav">
			<van-col span="24" class="spouse-nav-title">
				照片上传
			</van-col>
			<van-col span="12">
				<van-uploader class="spouse-fill"
					v-model="fileList1"
					:accept="'image/*'"
      ...
      <van-col span="12">
		...
		<div class="spouse-button-bottom" @click="submitFun()">保存</div>
	</div>
</template>
...
export default {
	...
	methods: {
		submitFun(){
			...
			// marryImg 结婚照  accountFirstImg 户口本首页   patientAccountImg 患者户口本照片
			if(_this.marryImg == ''){
				Toast("您还未上传结婚照！");
				return false
			}
			formData.append('marryImg',_this.marryImg);
      ...
      let toastUpload = Toast.loading({
        ...
        duration:0
            ...
            ary.push(obj);
						if(num == 0){
              _this.fileList1 = ary;
							_this.marryImg = response.data.data;
						}else if(num == 1){
              _this.fileList2 = ary;
...
#spouse .van-cell:not(:last-child):after{
    left: 0;
  }
</style>
